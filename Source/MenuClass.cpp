...
bool MenuClass::Option(const char* option)
{
	...
	if (Settings::currentOption <= Settings::maxVisOptions && Settings::optionCount <= Settings::maxVisOptions)
	{
		...
		onThis ? Drawing::Rect(Settings::OptionSelectedBackground, { Settings::menuX, (Settings::optionCount) * 0.035f + 0.1415f }, { Settings::menuWidth, 0.035f }) : NULL;
	}
	else if (Settings::optionCount > (Settings::currentOption - Settings::maxVisOptions) && Settings::optionCount <= Settings::currentOption)
	{
		...
		onThis ? Drawing::Rect(Settings::OptionSelectedBackground, { Settings::menuX,  (Settings::optionCount - (Settings::currentOption - Settings::maxVisOptions)) * 0.035f + 0.1415f }, { Settings::menuWidth, 0.035f }) : NULL;
	}
	if (onThis && Settings::selectPressed) { return true; }
	else { return false; }
}
...
void MenuClass::Checks::Keys()
{
	...
	if (GetTickCount() - Settings::keyPressPreviousTick > Settings::keyPressDelay) {
		if (GetAsyncKeyState(Settings::openKey)) { Settings::menuLevel == 0 ? MenuLevelHandler::MoveMenu(SubMenus::Main) : MenuLevelHandler::CloseMenu(); Settings::keyPressPreviousTick = GetTickCount(); }
		else if (GetAsyncKeyState(Settings::backKey)) { if (Settings::menuLevel > Nothing) { MenuLevelHandler::BackMenu(); } Settings::keyPressPreviousTick = GetTickCount(); }
		else if (GetAsyncKeyState(Settings::upKey)) { Settings::currentOption > 1 ? Settings::currentOption-- : Settings::currentOption = Settings::optionCount; Settings::keyPressPreviousTick = GetTickCount(); }
		else if (GetAsyncKeyState(Settings::downKey)) { Settings::currentOption < Settings::optionCount ? Settings::currentOption++ : Settings::currentOption = 1; Settings::keyPressPreviousTick = GetTickCount(); }
		else if (GetAsyncKeyState(Settings::leftKey)) { Settings::leftPressed = true; Settings::keyPressPreviousTick = GetTickCount(); }
		else if (GetAsyncKeyState(Settings::rightKey)) { Settings::rightPressed = true; Settings::keyPressPreviousTick = GetTickCount(); }
		else if (GetAsyncKeyState(Settings::selectKey)) { Settings::selectPressed = true; Settings::keyPressPreviousTick = GetTickCount(); }
	}
